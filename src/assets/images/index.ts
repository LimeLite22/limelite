export { default as AddOnlyIcon } from "./BottomMenu/AddOnlyIcon.svg";
export { default as EditOnlyIcon } from "./BottomMenu/EditOnlyIcon.svg";
export { default as BottomNewRequestIcon } from "./BottomMenu/NewRequestIcon.svg";
export { default as OtherIcon } from "./BottomMenu/OtherIcon.svg";
export { default as ShootEditIcon } from "./BottomMenu/ShootEditIcon.svg";
export { default as ShootOnlyIcon } from "./BottomMenu/ShootOnlyIcon.svg";
// header
export { default as SearchIcon } from "./Header/SearchIcon.svg";
// loader image
export { default as LoaderGif } from "./Loader/Logo.gif";
export { default as LoaderSvg } from "./Loader/Roll.svg";

// select profile
export { default as AddIcon } from "./SelectProfile/AddIcon.svg";
export { default as EditIcon } from "./SelectProfile/EditIcon.svg";
export { default as User1Foto } from "./SelectProfile/User1.png";
export { default as User2Foto } from "./SelectProfile/User2.png";
export { default as User3Foto } from "./SelectProfile/User3.png";
export { default as User4Foto } from "./SelectProfile/User4.png";

// sideBar
export { default as AddOns } from "./SideBar/AddOns.svg";
export { default as Home } from "./SideBar/Home.svg";
export { default as Inspiration } from "./SideBar/Inspiration.svg";
export { default as Learn } from "./SideBar/Learn.svg";
export { default as NewRequest } from "./SideBar/NewRequest.svg";
export { default as Notifications } from "./SideBar/Notifications.svg";
export { default as Projects } from "./SideBar/Projects.svg";
export { default as Settings2 } from "./SideBar/Settings.svg";
export { default as Support } from "./SideBar/Support.svg";

//support images
export { default as Arrow } from "./Support/Arrow.svg";
export { default as Arrow2 } from "./Support/Arrow2.svg";
export { default as AttachFileIcon } from "./Support/AttachFileIcon.svg";
export { default as ChatIcon } from "./Support/ChatIcon.svg";
export { default as SupportCloseIcon } from "./Support/CloseImage.svg";
export { default as EmailIcon } from "./Support/EmailIcon.png";
export { default as SendGreenIcon } from "./Support/SendGreenIcon.svg";
export { default as SendIcon } from "./Support/SendIcon.svg";
export { default as SettingsDots } from "./Support/SettingsDots.svg";
export { default as SmallMailIcon } from "./Support/SmallMailIcon.svg";
export { default as VoiceMessageIcon } from "./Support/VoiceMessageIcon.svg";

// notifications images
export { default as DoubleGap } from "./Notifications/DoubleGap.svg";
export { default as DoubleGapGray } from "./Notifications/DoubleGapGray.svg";

// welcome images
export { default as CompanyOverviewIcon } from "./Welcome/CompanyOverview.svg";
export { default as DemoVideosIcon } from "./Welcome/DemoVideos.svg";
export { default as DroneIcon } from "./Welcome/Drone.svg";
export { default as EducationVideo } from "./Welcome/EducationVideo.svg";
export { default as EventRecaps } from "./Welcome/EventVideo.svg";
export { default as FAQS } from "./Welcome/FAQs.svg";
export { default as Fundrasing } from "./Welcome/FundRaising.svg";
export { default as HRRecruiting } from "./Welcome/HRRecruiting.svg";
export { default as InfoIcon } from "./Welcome/InfoIcon.svg";
export { default as Interviews } from "./Welcome/Interviews.svg";
export { default as More } from "./Welcome/More.svg";
export { default as NewRequestIcon } from "./Welcome/NewRequestIcon.svg";
export { default as OnBoarding } from "./Welcome/OnBoarding.svg";
export { default as OpenedBurgerIcon } from "./Welcome/OpenedBurgerIcon.svg";
export { default as Photography } from "./Welcome/Photography.svg";
export { default as ProductServiceVideos } from "./Welcome/ProductServiceVideos.svg";
export { default as HypeSizzle } from "./Welcome/Sizzle.svg";
export { default as SocialMedia } from "./Welcome/SocialMedia.svg";
export { default as TelevisionCommercials } from "./Welcome/TelevisionCommercials.svg";
export { default as TestimonialsCaseStudies } from "./Welcome/TestimonialsCaseStudies.svg";
export { default as Videography } from "./Welcome/Videography.svg";
export { default as WebsiteHeaderVideos } from "./Welcome/WebsiteHeaderVideos.svg";

// walkThrough images
export { default as SwiperFoto1 } from "./WalkThrough/SwiperFoto1.webp";
export { default as SwiperFoto1Mob } from "./WalkThrough/SwiperFoto1Mob.webp";
export { default as SwiperFoto2 } from "./WalkThrough/SwiperFoto2.png";
export { default as SwiperFoto2Mob } from "./WalkThrough/SwiperFoto2Mob.webp";
export { default as SwiperFoto3 } from "./WalkThrough/SwiperFoto3.png";
export { default as SwiperFoto3Mob } from "./WalkThrough/SwiperFoto3Mob.webp";

// Welcome icons
export { default as AnnouncementVideos } from "./Welcome/AnnouncementVideos.svg";
export { default as ClosedBurgerIcon } from "./Welcome/ClosedBurgerIcon.svg";
export { default as ClosePopUpIcon } from "./Welcome/ClosePopUpIcon.svg";

// FAQs icons
export { default as Minus } from "./Faqs/Minus.svg";
export { default as Plus } from "./Faqs/Plus.svg";

// new request icons
export { default as CalendarIcon } from "./NewRequest/Calendar.svg";
export { default as CheckBox } from "./NewRequest/CheckBox.svg";
export { default as CheckBoxSelected } from "./NewRequest/CheckBoxSelected.svg";
export { default as CloseCalendar } from "./NewRequest/CloseCalendar.svg";
export { default as DirectionArrow } from "./NewRequest/DirectionArrow.svg";
export { default as Expand } from "./NewRequest/Expand.svg";
export { default as FileImage } from "./NewRequest/File.svg";
export { default as GrayArrow } from "./NewRequest/GrayArrow.svg";
export { default as Link } from "./NewRequest/Link.svg";
export { default as Note } from "./NewRequest/Note.svg";

// common
export { default as Close } from "./LearnMore/Close.svg";
export { default as Add } from "./Сommon/Add.svg";
export { default as Add2 } from "./Сommon/Add2.svg";
export { default as AddAccount } from "./Сommon/AddAccount.svg";
export { default as ArrowGray } from "./Сommon/ArrowGray.svg";
export { default as ArrowGray2 } from "./Сommon/ArrowGray2.svg";
export { default as ArrowGray3 } from "./Сommon/ArrowGray3.svg";
export { default as ArrowGray4 } from "./Сommon/ArrowGray3.svg";
export { default as ArrowLightGray } from "./Сommon/ArrowLightGray.svg";
export { default as ArrowWhite } from "./Сommon/ArrowWhite.svg";
export { default as Audio } from "./Сommon/Audio.svg";
export { default as BackCalendarArrow } from "./Сommon/BackCalendarArrow.svg";
export { default as Basket } from "./Сommon/Basket.svg";
export { default as ArrowBlue } from "./Сommon/BlueArrow.svg";
export { default as ArrowBlue3 } from "./Сommon/BlueArrowType3.svg";
export { default as CalendarIcon2 } from "./Сommon/Calendar.svg";
export { default as CalendarBlack } from "./Сommon/CalendarBlack.svg";
export { default as Chat2 } from "./Сommon/Chat.svg";
export { default as ChatSupport } from "./Сommon/ChatSupport.svg";
export { default as CloseRed } from "./Сommon/CloseRed.svg";
export { default as Copy } from "./Сommon/Copy.svg";
export { default as Delete } from "./Сommon/Delete.svg";
export { default as DeleteRed } from "./Сommon/DeleteRed.svg";
export { default as DetailsGreen } from "./Сommon/DetailsGreen.svg";
export { default as Divider } from "./Сommon/Divider.svg";
export { default as Download } from "./Сommon/Download.svg";
export { default as Drop } from "./Сommon/Drop.svg";
export { default as Edit } from "./Сommon/EditIcon.svg";
export { default as FaceBook } from "./Сommon/FaceBook.svg";
export { default as Filter } from "./Сommon/Filter.svg";
export { default as ForwardCalendarArrow } from "./Сommon/ForwardCalendarArrow.svg";
export { default as GapCheck } from "./Сommon/GapCheck.svg";
export { default as GapUnCheck } from "./Сommon/GapUnCheck.svg";
export { default as Instagram } from "./Сommon/Instagram.svg";
export { default as InvisiblePassword } from "./Сommon/InVisiblePassword.svg";
export { default as InviteUser } from "./Сommon/InviteUser.svg";
export { default as LastDays } from "./Сommon/LastDays.svg";
export { default as LearnMore } from "./Сommon/LearnMore.svg";
export { default as Linkedin } from "./Сommon/Linkedin.svg";
export { default as LocationBlack } from "./Сommon/Location.svg";
export { default as NoUser } from "./Сommon/NoUser.svg";
export { default as PNG } from "./Сommon/Png.svg";
export { default as QuestionIcon } from "./Сommon/QuestionIcon.svg";
export { default as QuestionLightGray } from "./Сommon/QuestionLightGray.svg";
export { default as Remove } from "./Сommon/Remove.svg";
export { default as Remove2 } from "./Сommon/Remove2.svg";
export { default as Search } from "./Сommon/Search.svg";
export { default as Settings } from "./Сommon/Settings.svg";
export { default as SettingsMenu } from "./Сommon/SettingsMenu.svg";
export { default as StatusApproved } from "./Сommon/StatusApproved.svg";
export { default as StatusProgress } from "./Сommon/StatusProgress.svg";
export { default as StatusUnavailable } from "./Сommon/StatusUnavailable.svg";
export { default as StatusApprovedBlack } from "./Сommon/StatusApprovedBlack.svg";
export { default as StatusProgressBlack } from "./Сommon/StatusProgressBlack.svg";
export { default as StatusUnavailableBlack } from "./Сommon/StatusUnavailableBlack.svg";
export { default as Success } from "./Сommon/Success.svg";
export { default as Success2 } from "./Сommon/Success2.svg";
export { default as SuccessGap } from "./Сommon/SuccessGap.webp";
export { default as Transfer } from "./Сommon/Transfer.svg";
export { default as Twitter } from "./Сommon/Twitter.svg";
export { default as Valid } from "./Сommon/Valid.svg";
export { default as VisiblePassword } from "./Сommon/VisiblePassword.svg";
export { default as Youtube } from "./Сommon/Youtube.svg";
export { default as ProjectOverviewImg } from "./Сommon/ProjectOverview.png";
// what is new

export { default as CompanyType } from "./WhatIsNew/CompanyType.svg";
export { default as EducationType } from "./WhatIsNew/EducationType.svg";
export { default as WhatisNew1 } from "./WhatIsNew/Slider1.png";
export { default as WhatisNew2 } from "./WhatIsNew/Slider2.png";
export { default as WhatisNew3 } from "./WhatIsNew/Slider3.png";
export { default as WhatisNew4 } from "./WhatIsNew/Slider4.png";
export { default as SocialMediaType } from "./WhatIsNew/SocialMediaType.svg";
